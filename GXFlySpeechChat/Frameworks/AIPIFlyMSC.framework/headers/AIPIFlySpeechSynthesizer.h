 ...
 *  设置识别的委托对象
 */
@property(nonatomic, weak) id<AIPIFlySpeechSynthesizerDelegate> delegate;
 ...
 */
- (BOOL)startSpeaking:(NSString *)text;

/**
 * 开始合成, 并根据需要设置是否自动播放
 * @param autoPlay YES:则自动播放; NO:则不会自动播放, 需要播放时需要手动调用beginPlay方法开始播放
 */
- (BOOL)startSynthesizer:(NSString *)text autoPlay:(BOOL)autoPlay;
/**
 * 仅用于 startSynthesizer:autoPlay: 方法中autoPlay=NO时, 开始播放合成音频
 */
- (void)beginPlay;
